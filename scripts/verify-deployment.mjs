...
await Promise.all(assetUrls(html).map(async (url) => {
...
const runs = await (await get(`https://api.github.com/repos/paste-perfect/paste-perfect/actions/workflows/ci.yml/runs?head_sha=${sha}&event=push&branch=dev&status=success`, headers)).json();
if (!runs.workflow_runs?.some((run) => run.head_sha === sha && run.head_repository.full_name === metadata.repository)) throw new Error("Deployed commit has no successful dev CI run.");
